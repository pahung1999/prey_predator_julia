...
const DEFAULT_LIFESPAN = Dict(:tiger => 10,  :boar => 3000,  :leopard => 20)
const DEFAULT_CATCH_PROB = Dict(:tiger => 0.3, :leopard => 0.5)

@kwdef struct ModelParams
	max_energy::Float16 = 1
	proba_reproduce::Float16 = 1
	max_offsprings::Int8 = 1
	energy_reproduce::Float16 = 1
	max_food::Float16 = 1
...
function agent_step!(agent, model, ::Val{:leopard})
	# Walk
	non_tiger_nearby=filter!(is_not_tiger, nearby_agents(agent, model, 2) |> collect)
	
	if agent.energy <0.2
		move_agent!(agent, rand(nearby_positions(agent.pos, model,2) |> Set), model)
	else
		move_agent!(agent, rand(non_tiger_nearby.pos), model)
	end
end
...
function agent_step!(agent, model, ::Val{:boar})
	...
	consumption = min(
		...
		params.max_energy - agent.energy
	)
...
function agent_step!(agent::Animal, model)
	...
	# Walk
	# move_agent!(agent, rand(nearby_positions(agent.pos, model) |> Set), model)
...
# ╔═╡ 5fc88461-606a-4ab5-a4d7-1eea209719fb
...
# ╔═╡ 2a5ce8cf-a3a8-4ea7-b69a-1ba47671c8ae
let 
	...
	adata, mdata = run!(model, agent_step!, model_step!, 10000; adata=adata, mdata=[healthy_food])
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "2959a7f55de1a284b96da54a03c3226713d3d3e0"
